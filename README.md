...
- **多环境一键启停**：自动分配空闲端口（8080+）、按环境配置强制使用 Node 版本（未配置时才自动探测）、自动匹配启动脚本，跨 Windows / macOS / Linux 拉起本地开发服务；指定版本不可用时直接停止，不会降级到系统 Node。Windows 下通过 fnm 执行 npm、npx、pnpm 或 yarn 时，会交给 `cmd.exe` 解析对应的 `.cmd` 入口，避免版本已切换但包管理器无法启动；Webpack 编译进度按百分比单行刷新并压缩落盘，同时关闭控制台快速选择暂停，避免误选文字后看似卡死。
