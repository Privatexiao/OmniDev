...
- **多环境一键启停**：自动分配空闲端口（8080+）、按环境配置强制使用 Node 版本（未配置时才自动探测）、自动匹配启动脚本，跨 Windows / macOS / Linux 拉起本地开发服务；指定版本不可用时直接停止，不会降级到系统 Node。Windows 下通过 fnm 执行 npm、npx、pnpm 或 yarn 时，会交给 `cmd.exe` 解析对应的 `.cmd` 入口，避免版本已切换但包管理器无法启动；复杂 PowerShell 启动脚本通过 `-EncodedCommand` 安全传递，npm、Vue CLI 与 Webpack 始终直接连接独立终端，保留项目默认的完整进度、警告和颜色，同时由进程级日志复制器将输出同步写入 UTF-8 日志，不再经过 PowerShell 逐行管道加工。终端同时关闭快速选择暂停，避免引号解析或误选文字造成启动闪退、看似卡死。
- **凭证安全注入**：登录凭证经 AES 加密存入本地保险库；本地纯 Cookie 登录通过一次性回环跳转在页面首次加载前写入系统默认浏览器，localStorage / sessionStorage / Header 则在 Playwright 独立浏览器首次导航前注入。
- **浏览器免密登录**：调起 Python + Playwright 自动识别表单并填密登录；需要本机可执行的 Python，Python 已安装时可自动补齐 Playwright 与 Chromium。
...
脚本会依次询问目标版本号和中文更新说明，然后自动修改并同步版本、判断更新模式、执行 Vue/JavaScript 与已知问题回归检查、签名打包、查找 NSIS `.exe.sig`、回填签名和执行最终校验。主版本号变化默认使用 `manual`，其余版本默认使用 `automatic`；高风险版本可显式覆盖：
...
更新提示和通用弹窗的主题样式位于 Vue 作用域样式中。组合全局主题选择器时，必须将完整选择器写入 `:global(...)`，例如 `:global([data-theme="light"] .update-banner-title)`；不要写成 `:global([data-theme="light"]) .update-banner-title`，否则旧版 Vue SFC 编译器可能只保留根主题选择器，使 `-webkit-text-fill-color` 等可继承属性污染整个页面。
...
4. **凭证存储与本地边界**：敏感凭证 AES 加密落库、环境配置物理文件脱敏；需要执行注入时，明文只通过绑定在 `127.0.0.1` 的本地控制接口提供给当前应用。终端日志使用 Vue 安全插值，禁用 `v-html`。
...
3. **自动登录依赖**：纯 Cookie 本地登录不依赖 Python；localStorage / sessionStorage / Header 或表单自动登录要求本机先能执行 `python`，随后脚本可自动补齐 Playwright 与 Chromium 内核。
